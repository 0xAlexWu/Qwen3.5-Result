...
def plot_nested_grouped_bars(df, category_col, metrics, title, ylabel, stem, category_order=None):
    ...
    for cat_i, cat in enumerate(category_order):
        ...
        for met_i, (metric_col, metric_name) in enumerate(metrics):
            for mod_i, model in enumerate(MODEL_ORDER):
                ...
                label = None
                if cat_i == 0:
                    label = f"{metric_name} - {model}"

                if pd.notna(value):
                    plt.bar(xpos, value, width=bar_width, label=label)
...
plot_train_line(
    ...
    "Token accuracy",
    "comparison_train_token_accuracy_only"
)
...
plot_nested_grouped_bars(
    ...
    "Split-level Structured Output Metrics Comparison",
    "Rate",
    "comparison_split_level_structured_metrics",
    split_order,
)
...
plot_nested_grouped_bars(
    ...
    "Metrics by Resource Type Comparison",
    "Rate",
    "comparison_metrics_by_resource_type",
    resource_order,
)
...
plot_nested_grouped_bars(
    ...
    "Metrics by Source Dataset Comparison",
    "Rate",
    "comparison_metrics_by_source_dataset",
    dataset_order,
)
...
plot_nested_grouped_bars(
    ...
    "Observation Metrics by Split Comparison",
    "Rate",
    "comparison_observation_metrics_by_split",
    split_order,
)

print("Done. Generated comparison figures in:", OUT_DIR)
